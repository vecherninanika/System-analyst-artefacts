## 1. Артефакты со стажировки в T1

Бизнес-требования: [`Бизнес требования Т1.docx`](Бизнес_требования_Т1.docx)
Системные требования: [`Системные требования_T1.docx`](Системные_требования_T1.docx)
ER-диаграмма БД: [`openapi_T1.yaml`](openapi_T1.yaml)
API-документация в Swagger: [`openapi_T1.yaml`](openapi_T1.yaml)

## 2. BPMN-диаграмма процесса заказа доставки продуктов

![BPMN-диаграмма процесса заказа доставки продуктов](bpmn_заказ_в_лавке.drawio.png)

Описывает сценарий заказа доставки продуктов пользователем. Использовала нотацию BPMN 2.0.

## 3. ER-модель базы данных

![ER-модель системы аренды инвентаря](ER_inventory_nika.drawio.png)

Отображает основные сущности: `Inventory`, `User`, `Booking Journal` и другие и связи между ними (1-to-1, 1-to-many, many-to-many). Спроектирована в Drawio.

## 4. UML-диаграмма последовательностей

![UML-диаграмма последовательностей](sequence_diagram_оформление_заказа.png)

Показывает, как запрос на оформление заказа с фронта проходит путь через все компоненты системы. Код написан в PlantUML.

Файл с кодом: [`plantuml_оформление_заказа.puml`](plantuml_оформление_заказа.puml)

## 5. Коллекция запросов API в Postman

![Коллекция запросов API в Postman](postman_dummy_json.jpg)

*Все основные запросы к тестовому API DummyJSON с описанием и сохраненными примерами успешных и неуспешных ответов.*

![Сохраненный пример ответа](postman_dummy_json_get_200.jpg)

*Сохраненный пример успешного ответа на запрос GET /posts с пагинацией.*

Файл с коллекцией: [`DummyJSON_test.postman_collection.json`](DummyJSON_test.postman_collection.json)


**Инструменты:** Drawio, PlantUML, Postman
